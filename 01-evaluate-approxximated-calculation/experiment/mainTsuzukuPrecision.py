...
def evaluate_diag(origin, data, num_sample):
	measure = {}
	...
	model 		= copy.deepcopy(origin)
	time_start	= time.perf_counter()
	tsuzukuArxiv	= hack.tsuzuku_arxiv(data, model, num_sample=num_sample)
	time_end		= time.perf_counter()

	report(f'\tEffective time:{time_end - time_start: 9.4f} sec')
	measure['arxiv-outer-time'] = time_end - time_start
	
	model 		= copy.deepcopy(origin)
	time_start	= time.perf_counter()
	tsuzukuICML	= hack.tsuzuku_icml(data, model, num_sample=num_sample)
	time_end	= time.perf_counter()

	report(f'\tEffective time:{time_end - time_start: 9.4f} sec')
	measure['icml-outer-time'] = time_end - time_start

	report(f'\tTr[CorrectH]   :{correctH.sum().item(): 9.4f}')
	
#	measure['correct']  = correctH.detach().cpu()
#	measure['arxiv'] 	= tsuzukuArxiv
#	measure['icml'] 	= tsuzukuICML
	
	correctH  = correctH.detach().cpu()

	tsuzukuArxiv['diffs'] = []
	for i,diagH in enumerate(tsuzukuArxiv['diagHs']):
		diff = (correctH - diagH)
		diff = (diff**2).sum().sqrt()
		tsuzukuArxiv['diffs'].append(float(diff))
	del tsuzukuArxiv['diagHs']
	measure['arxiv'] =  tsuzukuArxiv


	tsuzukuICML['diffs'] = []
	for i,diagH in enumerate(tsuzukuICML['diagHs']):
		diff = (correctH - diagH)
		diff = (diff**2).sum().sqrt()
		tsuzukuICML['diffs'].append(float(diff))
	del tsuzukuICML['diagHs']
	measure['icml'] =  tsuzukuICML
...
def get_config():
	...
	args.add_argument('--num_sample',		default=10,			type=int)	# need to use same data
...
def main():
	...
	measure = evaluate_diag(model, data, config.num_sample)
